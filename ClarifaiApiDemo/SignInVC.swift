...
class SignInVC: UIViewController, FBSDKLoginButtonDelegate {
    ...
    */
    func loginButton(loginButton: FBSDKLoginButton!, didCompleteWithResult result: FBSDKLoginManagerLoginResult!, error: NSError!) {
        ...
        let optionPage = self.storyboard?.instantiateViewControllerWithIdentifier("OptionsVC") as! OptionsVC
        
        let optionPageNav = UINavigationController(rootViewController: optionPage)
        let appDelegate = UIApplication.sharedApplication().delegate as! AppDelegate
        
        appDelegate.window?.rootViewController = optionPageNav
    }
